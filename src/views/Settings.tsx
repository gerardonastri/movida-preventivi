...
export default function Settings() {
  ...
  const handleExportExcel = () => {
    try {
      ...
      if (quotes.length === 0) { showToast("Nessun preventivo da esportare", "info"); return; }
    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-12 relative">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-[var(--shadow-card)] border border-[var(--border)] space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          {/* FIX: Testo fattura elettronica configurabile */}
          <div className="md:col-span-2">
            <label className={labelClass}>
              Testo "Richiesta Fattura Elettronica" (stampato nel PDF)
            </label>
            <textarea
            ...
            <p className="text-xs text-[var(--text-muted)] mt-1 ml-1">
              Questo testo appare in grassetto nella sezione pagamento del documento PDF.
            </p>
          </div>
          ...
          <div className="md:col-span-2 bg-[var(--bg-tertiary)] p-4 rounded-xl mt-2 border border-[var(--border)]">
            ...
            <p className="text-xs text-[var(--text-muted)] mt-2">
              Scegli un'immagine dal tuo PC. Verrà salvata in memoria e usata per il PDF.
            </p>
            {settings.logoBase64 && (
              <div className="mt-4">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-[var(--shadow-card)] border border-[var(--border)] space-y-4 mt-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)] mt-0.5">
              Scarica i tuoi preventivi in vari formati o esegui un backup completo del sistema.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-[var(--border)]">
          ...
          <div className="flex flex-col gap-2">
            <motion.button
              ...
              className={`w-full py-2.5 rounded-xl font-medium transition flex items-center justify-center gap-2 border ${isGeneratingPdf ? 'bg-gray-100 text-gray-400 border-gray-200' : 'bg-red-50 hover:bg-red-100 text-red-700 border-red-200'}`}
            >
        ...
        <div className="pt-4 mt-2 border-t border-[var(--border)] flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-[var(--text-secondary)]">
            Hai cambiato dispositivo? Ripristina un backup JSON salvato in precedenza.
          </p>
          <motion.button
            whileTap={{ scale: 0.97 }}
