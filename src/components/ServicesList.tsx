...
export default function ServicesList({ services, onChange }: ServicesListProps) {
  ...
  const handleAddCustom = () => {
    onChange([...services, { id: crypto.randomUUID(), name: '', qty: 1, unitPrice: 0 }]);
  };
          ...
          {catalog.map(cat => (
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${
                selectedCategory === cat.category ? 'bg-[var(--accent)] text-white shadow-sm' : 'bg-white text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)]'
              }`}
            >
              {cat.category}
            </button>
        ...
        <Reorder.Group axis="y" values={services} onReorder={onChange} className="space-y-3">
          {services.map((item) => (
            <Reorder.Item 
              key={item.id} 
              value={item}
              className="flex items-center gap-2 md:gap-3 bg-white border border-[var(--border)] p-2 rounded-xl group relative hover:shadow-md transition-shadow"
            >
              ...
              <div className="flex-1">
                <input type="text" value={item.name} onChange={(e) => updateService(item.id, 'name', e.target.value)} placeholder="Descrizione..." className={inputClass} />
              </div>
