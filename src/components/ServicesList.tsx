...
export default function ServicesList({ services, onChange }: ServicesListProps) {
  ...
  const handleAddCustom = () => {
    onChange([...services, { id: crypto.randomUUID(), name: '', details: '', notes: '', qty: 1, unitPrice: 0 }]);
  };
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-[var(--shadow-card)] border border-[var(--border)] space-y-6">
      ...
      {/* Database Catalogo (Cliccabile) */}
      <div className="bg-[var(--bg-primary)] p-4 rounded-xl border border-[var(--border)] max-h-56 overflow-y-auto">
        <div className="flex justify-between items-center mb-3">
          <p className="text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider">Aggiungi dal Catalogo</p>
          <span className="text-[10px] bg-[var(--bg-tertiary)] px-2 py-1 rounded-md text-[var(--text-muted)]">
            Gestisci i pacchetti nella pagina "Catalogo"
          </span>
        </div>
        
        {catalog.length === 0 ? (
          <p className="text-sm text-[var(--text-muted)] italic text-center py-2">
            Il catalogo è vuoto. Clicca su "+ Aggiungi Riga Vuota" qui sotto per fare tutto a mano.
          </p>
        ) : (
      ...
      <div className="space-y-3">
        {services.length === 0 && <p className="text-sm text-[var(--text-muted)] text-center py-4 italic">Nessun servizio nel preventivo.</p>}
            ...
            <Reorder.Item key={item.id} value={item} className="flex flex-col gap-2 bg-white border border-[var(--border)] p-3 rounded-xl group relative hover:shadow-md transition-shadow">
              ...
              <div className="flex items-center gap-2 w-full">
                ...
                <div className="flex-1">
                  <input type="text" value={item.name} onChange={(e) => updateService(item.id, 'name', e.target.value)} placeholder="Nome Servizio (Testo Nero in PDF)..." className={inputClass + " font-bold uppercase text-xs"} />
                </div>
                ...
                <div className="w-24 relative">
                  ...
                  <input type="number" min="0" step="0.01" value={item.unitPrice} onChange={(e) => updateService(item.id, 'unitPrice', Number(e.target.value))} className={`${inputClass} pl-6 font-bold`} />
                </div>
                
                <button onClick={() => removeService(item.id)} className="w-8 h-8 flex flex-shrink-0 items-center justify-center text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition">
                  ✕
                </button>
              </div>
              
              <div className="pl-8 pr-10 w-full flex flex-col gap-2">
                {/* Fallback di sicurezza: se il preventivo era stato salvato prima dell'update con "description", mostralo in "details" */}
                <textarea 
                  value={item.details !== undefined ? item.details : ('description' in item ? (item as QuoteService & { description: string }).description : '') || ''} 
      ...
      <button onClick={handleAddCustom} className="w-full py-3 rounded-xl border-2 border-dashed border-[var(--border)] text-[var(--text-secondary)] font-medium hover:bg-[var(--bg-tertiary)] hover:text-[var(--text-primary)] transition flex items-center justify-center gap-2 text-sm">
        + Aggiungi Riga Vuota
      </button>
    </div>
  );
